...
// SetFakeIPRanges replaces the prefixes this responder answers for, and reports
// whether anything changed. Without it a config reload that moves
// dns.fake-ip-range would leave the responder on the old prefix: it would stop
// answering the new fake addresses and start forging replies for real hosts
// that happen to sit in the range the fake pool just vacated.
func (b *FakeIPICMPBackend) SetFakeIPRanges(ipv4 netip.Prefix, ipv6 netip.Prefix) (bool, error) {
	if b == nil {
		return false, nil
	}
	...
	previous := b.control
	b.control.Flags &^= fakeIPICMPFlagFakeIPv4 | fakeIPICMPFlagFakeIPv6
	...
	if normalizedIPv4.IsValid() {
		b.control.Flags |= fakeIPICMPFlagFakeIPv4
	...
	if normalizedIPv6.IsValid() {
		b.control.Flags |= fakeIPICMPFlagFakeIPv6
	...
	zero := uint32(0)
	if err = updateMap(b.controlFD, unsafe.Pointer(&zero), unsafe.Pointer(&b.control)); err != nil {
		b.control = previous
		return false, E.Cause(err, "update fakeip_icmp eBPF control")
	}
	return true, nil
}
