...
`common/ebpf/internal/bpfgen/*_bpfel.o` and `*_bpfeb.o` are generated by bpf2go
from the sources in `common/ebpf/native/` and are committed together with a
`manifest.txt` recording the toolchain and flags they were built with.
CI runs:
