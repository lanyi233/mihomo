...
// Retry-scheduler cases adapted from CHIZI-0618/sing-box.
package sing_ebpf
...
func newRetryLoopHarness(t *testing.T) *retryLoopHarness {
	...
	go func() {
		...
		// This harness and every test built on it only ever cares about the
		// sharedRewrite component; general and bypassRuleSet are pinned to
		// Settled so they never affect the arm/disarm sequence these tests
		// assert on. interface_monitor_retry_components_test.go covers those two
		// components' own independence directly.
...
// The loop's whole contract rests on Unknown meaning "this step did not run",
// so a pass that returns before any step has to report exactly that. Reporting
// Settled here instead would silently disarm a recovery every time a cancelled
// pass raced a pending one, and every loop-level test would still pass because
// they all feed the loop a canned outcome.
func TestUpdateTCInterfacesReportsNothingWhenCancelled(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	outcome := (&Inbound{}).updateTCInterfaces(ctx)

	if outcome != (tcUpdateOutcome{}) {
		t.Fatalf("cancelled pass reported %+v, want every component unknown", outcome)
	}
	if tcSharedRewriteUnknown != 0 {
		t.Fatal("the zero tcUpdateOutcome is only all-unknown while Unknown is the zero outcome")
	}
}

// Arm after a delay has already elapsed must not deliver the old deadline: the
// loop treats a wakeup as "this component's retry is due now", so a leaked
// signal runs a pass the scheduler never scheduled.
func TestRealRetryTimerDropsTheDeadlineItReplaced(t *testing.T) {
	timer := newTCRetryTimer()
	defer timer.Disarm()
	...
	// Fire again without draining, then re-arm long: nothing may be pending.
	timer.Arm(time.Millisecond)
	time.Sleep(20 * time.Millisecond)
	timer.Arm(time.Hour)
	select {
	case <-timer.Expired():
		t.Fatal("re-arming delivered the deadline it replaced")
	...
	select {
	case <-timer.Expired():
		t.Fatal("disarm left a signal pending")
	default:
	}
}
