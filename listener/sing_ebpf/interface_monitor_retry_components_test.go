...
// Retry-scheduler component cases adapted from CHIZI-0618/sing-box.
package sing_ebpf
...
// componentRetryLoopHarness is retryLoopHarness's sibling for tests that need
// independent control over all three tcUpdateOutcome components, not just
// sharedRewrite. It shares the same fake-timer substitution and the same
// round-synchronization contract (one timer action per round).
type componentRetryLoopHarness struct {
	...
	// before the loop has armed or disarmed even once, a non-nil zero time when
	// the loop last reported disarmed.
	schedule *time.Time
}
...
// nextRetryAt reports the harness's most recently observed schedule value, the
// same way a reporter fed by the loop's own hook would see it -- nil
// (armed/disarmed unknown) before any round has completed, a zero time when
// disarmed, or the armed deadline.
...
func newComponentRetryLoopHarness(t *testing.T) *componentRetryLoopHarness {
	...
	// A drift check this short would otherwise fire spuriously mid-test; tests
	// that want to exercise it set it back down explicitly.
...
// TestRetryLoopGeneralComponentBacksOffIndependently proves the general bucket
// (TC attachment/infrastructure/host policy) gets the same real exponential
// backoff sharedRewrite already had, and settles on its own.
...
// for the bypass_rule_set component specifically, since it is normally driven
// by rule-provider update callbacks rather than network events -- the scheduler
// is what gives it a retry path at all when nothing else changes.
...
// TestRetryLoopComponentsAreIndependent ensures one component's recovery cannot
// clear or disturb another component's pending retry.
func TestRetryLoopComponentsAreIndependent(t *testing.T) {
	...
	// proving its state was actually cleared when it settled rather than merely
	// being masked by general's still-outstanding one.
...
// TestRetryLoopReportsNextRetryTime proves the onScheduleChange hook actually
// reflects the loop's own timer, not a value guessed at independently: nil
// before the loop has armed or disarmed even once, the real deadline while a
// component is outstanding, and the zero time once every component has settled
// and the timer is disarmed again.
...
// TestRetryLoopDriftCheckRunsWithNothingOutstanding proves the periodic,
// unconditional drift check: with every component settled and no retry timer
// armed at all, a tick on the drift ticker still drives an update pass -- the
// scenario a purely event- and retry-driven scheduler would never reach on its
// own, which is exactly the drift-with-no-event case (Android's netd flushing
// the clsact filters between network changes) this exists to catch.
func TestRetryLoopDriftCheckRunsWithNothingOutstanding(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	timer := &testRetryTimer{
		expired: make(chan time.Time, 1),
	...
	case <-ran:
	case <-time.After(2 * time.Second):
		t.Fatal("the drift check never drove an update with nothing outstanding and no netlink events")
	}
	// A second tick proves it as a real recurring check, not a one-shot artifact
	// of loop startup.
	select {
	case <-ran:
	case <-time.After(2 * time.Second):
		t.Fatal("the drift check did not recur")
	}
}
